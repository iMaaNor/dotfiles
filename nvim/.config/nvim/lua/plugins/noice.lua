return {
	...
	config = function()
		require("noice").setup({
			...
			routes = {
				-- Disable "written" messages (file saved)
				{
					filter = { event = "msg_show", find = "written" },
					opts = { skip = true },
				},
				-- Disable "X lines deleted" messages
				{
					filter = { event = "msg_show", find = " lines? deleted" },
					opts = { skip = true },
				},
				-- Disable "X more lines" or "X fewer lines" messages (undo/redo)
				{
